 ...
 *   6. Когда ок — Run → installExportTrigger — раз в 10 минут.
 *
 * Важно про частоту: раз в минуту (было раньше) упирается в дневную квоту
 * Apps Script на UrlFetchApp — при 7 стадиях воронки × пагинации 1440
 * прогонов/день быстро выжирают лимит, экспорт встаёт на весь остаток дня
 * (квота сбрасывается по тихоокеанскому времени). Раз в 10 минут — тот же
 * трафик в 10 раз меньше; мгновенное обновление статуса и так идёт через
...
/**
 * Запусти один раз вручную — ставит опрос раз в 10 минут (страховка на
 * случай пропущенного webhook + подхват новых лидов, см. doPost ниже).
 *
 * Было каждую минуту — упёрлось в дневную квоту Apps Script на UrlFetchApp
 * («Service invoked too many times for one day: urlfetch», сбрасывается по
 * тихоокеанскому времени, поэтому экспорт вставал каждый день в одно и то
 * же время по Ташкенту и не восстанавливался до следующего дня). При 7
 * стадиях воронки × пагинации 1440 прогонов/день быстро выжирали лимит.
 * Раз в 10 минут — тот же трафик в 10 раз меньше, мгновенное обновление
 * статуса и так идёт через doPost, эта функция только страховка.
 */
function installExportTrigger() {
  ScriptApp.getProjectTriggers()
    .filter((t) => t.getHandlerFunction() === 'exportOnce')
    .forEach((t) => ScriptApp.deleteTrigger(t));
  ScriptApp.newTrigger('exportOnce').timeBased().everyMinutes(10).create();
  Logger.log('Триггер поставлен: exportOnce раз в 10 минут.');
}
