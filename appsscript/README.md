...
Меняется командами боту в Telegram — без правки кода. Работает только для
одного заранее заданного администратора (`ADMIN_TELEGRAM_USER_ID`) — от
всех остальных бот такие команды молча игнорирует.

### Разовая настройка

1. Узнай свой Telegram `user_id` — то же самое `getUpdates`, что и раньше
   (шаг 1 выше), поле `message.from.id` у твоего сообщения.
2. Script Properties → добавь `ADMIN_TELEGRAM_USER_ID` = твой `user_id`.
3. Выбери `installTelegramWebhook` в списке функций → **Run** — один раз.
   Сгенерирует секрет (`TELEGRAM_WEBHOOK_SECRET`) сам и подключит webhook
   к этому Web App. Безопасно перезапускать.
4. Напиши боту `/gettemplate` (в личку или в ту же группу) — должен
