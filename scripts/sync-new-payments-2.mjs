...
const PAYMENTS = [
  { phone: '998935955708', amount: 190000, method: 'cash', group: 'I12', date: '2026-08-06' },
  { phone: '998935955708', amount: 700000, method: 'uzcard', group: 'I12', date: '2026-08-06' },
];
...
for (const p of PAYMENTS) {
  ...
  if (sSnap.empty) { console.log(`${p.phone}: не найден`); continue; }
  const student = { id: sSnap.docs[0].id, ...sSnap.docs[0].data() };
  const group = groupsByCode.get(p.group);
  console.log(`${APPLY ? 'ПЛАТЁЖ' : 'dry-run'}: ${student.fullName} ${p.amount} ${p.method} ${p.group} ${p.date}`);
  if (APPLY) {
    await recordPayment(
      db,
      { student, branchId: BRANCH_ID, amount: p.amount, method: p.method, date: new Date(`${p.date}T00:00:00`), comment: '', groupId: group?.id ?? null, groupCode: group?.code ?? null },
      staffUser,
    );
  }
}
