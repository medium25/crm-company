...
const outside = students.filter((s) => !inList(s));
console.log(`Всего активных студентов (не архив): ${students.length}`);
console.log(`Вне списка "без истории списаний": ${outside.length}\n`);

outside
  .sort((a, b) => (a.fullName || '').localeCompare(b.fullName || ''))
  .forEach((s) => {
    const charges = (chargesByStudent.get(s.id) ?? []).map((c) => `${c.month}:${c.amount}`).join(', ');
    console.log(`${(s.fullName || '').padEnd(30)} phone=${s.phone}  balance=${String(s.balance).padStart(10)}  charges=[${charges}]`);
  });
process.exit(0);
